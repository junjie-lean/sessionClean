...
function renderCacheList() {
  cacheListEl.innerHTML = CACHE_ITEMS.map((item) => `
    <li class="cache-item" data-id="${item.id}">
      ...
      <input id="${item.id}" type="checkbox" class="item" checked />
    </li>
  `).join("");
            ...
            request.onsuccess = (event) => {
              ...
              storeNames.forEach((storeName) => {
                ...
                cursorReq.onsuccess = (cursorEvent) => {
                  ...
                  if (cursor) {
                    total += JSON.stringify(cursor.key).length * 2;
...
async function saveHistory(options, releasedBytes) {
  const record = {
    time: Date.now(),
    url: currentTab.url,
    options,
...
async function showHistory() {
  ...
  const time = new Date(latest.time).toLocaleString();
  setStatus(`最近清理：${time}，释放 ${formatSize(latest.releasedBytes)}`);
}
...
confirmBtn.addEventListener("click", async () => {
  ...
  const releasedBytes = ITEM_IDS.reduce((sum, id) => {
    return options[id] ? sum + (cacheStats[id] || 0) : sum;
  }, 0);
  ...
  try {
    ...
    await saveHistory(options, releasedBytes);
    setStatus("清理完成");
    await loadStats();
  } catch (err) {
